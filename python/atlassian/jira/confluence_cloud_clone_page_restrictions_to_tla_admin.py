...
def get_current_user_account_id(session: Optional[requests.Session], rest_root: str) -> Optional[str]:
    """Best-effort: confirm auth works and retrieve the caller's accountId."""
    if session is None:
        return None
    try:
...
def request_with_admin_retry(
    primary: requests.Session,
    admin: Optional[requests.Session],
    method: str,
    url: str,
    *,
    params=None,
    ...
    """Perform a request; if 401/403 and admin session provided, retry with admin session."""
    r = primary.request(method, url, params=params, headers=headers, data=data, timeout=timeout)
    if admin is not None and r.status_code in (401, 403):
        r = admin.request(method, url, params=params, headers=headers, data=data, timeout=timeout)
    return r
...
def resolve_account_id_best_effort(session: requests.Session, rest_root: str, query: str) -> Optional[str]:
    ...
    r = request_with_admin_retry(primary, admin, 'GET', url, params=params, timeout=60)
    if r.status_code != 200:
        return None
...
def iter_space_pages(primary: requests.Session, admin: Optional[requests.Session], rest_root: str, space_key: str, sleep_ms: int = 100):
    """
    ...
    while True:
        ...
        r = request_with_admin_retry(primary, admin, 'GET', url, params=params, timeout=90)
        r.raise_for_status()
        data = r.json()
...
def add_user_to_restriction(
    ...
    r = request_with_admin_retry(primary, admin, 'PUT', url, params=params, timeout=60)
    if r.status_code == 200:
        return True, "OK"
...
def main() -> int:
    ...
    ap.add_argument("--admin-section", required=False, help="Optional INI section name for a Confluence/Site admin account to access restricted content (e.g., VUMC_Cloud_Atlassian_Admin). If provided, the script will retry 401/403 calls with this account.")
    ...
    primary_session = make_session(inst)

    admin_session = None
    if args.admin_section:
        admin_inst = read_ini(args.ini, args.admin_section)
        admin_session = make_session(admin_inst)

    # Sanity-check who we're authenticated as (useful to confirm the admin token is valid)
    primary_account = get_current_user_account_id(primary_session, rest_root)
    admin_account = get_current_user_account_id(admin_session, rest_root)
    if args.admin_section and not admin_account:
        print("WARNING: --admin-section was provided but /user/current did not return 200 for that session. Check the INI credentials/token.")
    if not primary_account:
        print("WARNING: Primary session auth could not be validated via /user/current. Continuing anyway...")

    # Prefer admin session for user lookups if provided (visibility can differ)
    lookup_session = admin_session if admin_session else primary_session

    mapping_rows = read_mapping_csv(args.mapping_csv)
    mapping = build_mapping(mapping_rows, lookup_session, rest_root)

    ops = ["read", "update"]  # Confluence restriction operations 
    report_rows: List[Dict[str, str]] = []

    for page in iter_space_pages(primary_session, admin_session, rest_root, args.spacekey, sleep_ms=args.sleep_ms):
        ...
        for op_key in ops:
            restricted_users = get_restriction_users_for_operation(primary_session, admin_session, rest_root, page_id, op_key, sleep_ms=max(50, args.sleep_ms // 3))
            ...
            for src_account in restricted_users:
                ...
                ok, msg = add_user_to_restriction(primary_session, admin_session, rest_root, page_id, op_key, tla_account, dry_run=args.dry_run)
                report_rows.append({
                    "pageId": page_id,
                    "title": title,
                    "operation": op_key,
    ...
    print("NOTE: Adding restrictions requires permission to edit each page being modified. Use --admin-section to retry 401/403 requests with a Confluence/Site admin account.")
    return 0
