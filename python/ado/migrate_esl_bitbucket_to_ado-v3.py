...
# ---------------- Resiliency & Preflight ----------------
def _mask_mid(s: Optional[str]) -> str:
    if not s: return "<empty>"
    s = str(s)
    return s[:3] + "****" + s[-3:] if len(s) > 6 else "****"

def ado_pat_preflight(org: str, pat: str) -> Dict[str, Tuple[bool, str]]:
    """
    Probe ADO endpoints to validate PAT scopes. Build probe is non-blocking if TLS flakes.
    """
    base = f"https://dev.azure.com/{org}/_apis"
    endpoints = {
        "code": f"{base}/git/repositories?api-version=7.0",
        "projteam": f"{base}/projects?api-version=7.0",
        "build": f"{base}/build/definitions?api-version=6.0"
    }
    ...
    for name, url in endpoints.items():
        try:
            ...
            # Treat build TLS issues as non-blocking; others still helpful info
            msg = f"TLS error: {e}"
            if name == "build":
                results[name] = (False, msg + " (non-blocking)")
            else:
...
def configure_git_repo(repo_dir: Path, git_exe: str, prefer_ipv4: bool, enable_diag: bool):
    # Use Windows cert store on Git for Windows
    sh([git_exe, "config", "http.sslBackend", "schannel"], cwd=str(repo_dir))
    # Force HTTP/1.1 initially to avoid some proxies' HTTP/2 resets
    sh([git_exe, "config", "http.version", "HTTP/1.1"], cwd=str(repo_dir))
    if enable_diag:
        os.environ["GIT_TRACE"] = "1"
        os.environ["GIT_CURL_VERBOSE"] = "1"
    if prefer_ipv4:
        # Not an official knob, but useful indicator in traces; real IPv4 pref is in apply_force_ipv4()
        os.environ["GIT_USE_IPV4"] = "1"

def _is_transient_git_err(stderr_text: str) -> bool:
    st = (stderr_text or "").lower()
    signatures = [
        "connection was reset", "recv failure", "connection reset by peer",
        "schannel: next initialize security context failed",
        "the ssl connection could not be established",
        "http/2 stream", "failed to connect to dev.azure.com port 443",
        "couldn't connect to server", "network is unreachable", "timeout"
    ]
    return any(sig in st for sig in signatures)

def push_with_retries(repo_dir: Path, git_exe: str, dest_url: str, pat: str, max_attempts: int = 3) -> Tuple[bool, str]:
    """
    Push mirror with a small retry matrix. Uses Basic via http.extraHeader to avoid prompt.
    """
    ado_basic = make_basic("", pat)
    variants = [
        ([], {}),
        (["-c", "http.version=HTTP/1.1"], {}),
        (["-c", "http.version=HTTP/1.1"], {"GIT_SSL_NO_REVOKE": "1"})
    ]
    # Ensure push remote is set
    sh([git_exe, "remote", "set-url", "--push", "origin", dest_url], cwd=str(repo_dir))
    last_err = ""
    for i, (args, envadds) in enumerate(variants[:max_attempts], start=1):
        env = os.environ.copy(); env.update(envadds)
        print(f"[git push attempt {i}] variant={args or 'default'} env_vars={','.join(envadds.keys()) or 'none'}")
        rc, out, err = sh([git_exe, *args, "-c", f"http.extraHeader=Authorization: Basic {ado_basic}", "push", "--mirror"], cwd=str(repo_dir), env=env)
        if rc == 0:
            return True, "mirror push: OK"
        last_err = (err or "") + (out or "")
        print(f"[git push attempt {i}] failed rc={rc}. stderr/stdout:\n{last_err}")
        if not _is_transient_git_err(last_err) and i == 1:
            # very likely auth/permission
            if "authentication failed" in last_err.lower() or "http 403" in last_err.lower():
                return False, "mirror push: AUTH/403"
        # small backoff
        time.sleep(min(5 * i, 10))
    return False, f"mirror push: exhausted retries. Last error: {last_err[:600]}"
...
def main():
    ...
    ap.add_argument("--git-diagnostics", action="store_true")
    ap.add_argument("--ado-pat")
    ...
    ado_pat = (args.ado_pat or cfg.get("ado", "pat", fallback="")).strip()
    ...
    # --- ADO PAT preflight (non-blocking for build TLS flukes) ---
    if not (args.dry_run or args.ado_offline):
        print("== ADO PAT preflight ==")
        probes = ado_pat_preflight(ado_org, ado_pat)
        for k, (ok, msg) in probes.items():
            print(f"  {k:9}: {'PASS' if ok else 'FAIL'} ({msg})")
        if not (probes.get("code", (False,''))[0] and probes.get("projteam", (False,''))[0]):
            print("[ERROR] PAT preflight failed for Code/Project endpoints. Fix scopes/network and retry.", file=sys.stderr)
            sys.exit(2)
    ...
    for row in repo_rows:
        ...
        if args.dry_run:
            ...
            if rc != 0:
                ...
                # Harden repo git settings & attempt resilient push
                configure_git_repo(mirror_dir, git_exe, args.force_ipv4, args.git_diagnostics)
                ok, msg = push_with_retries(mirror_dir, git_exe, dest_url, ado_pat, max_attempts=3)
                print(msg)
                push_ok = ok
